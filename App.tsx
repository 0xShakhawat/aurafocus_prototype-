...
const BottomNavBar: React.FC<{ activeScreen: Screen; navigateTo: NavigateTo }> = ({ activeScreen, navigateTo }) => {
    const navItems = [
        ...
        { screen: Screen.ManageApps, icon: 'settings', label: 'Settings', isPlaceholder: true },
    ];
        ...
        <nav className="sticky bottom-0 z-30 bg-card-dark/80 backdrop-blur-sm border-t border-white/10">
            <div className="mx-auto flex max-w-md justify-around p-2">
                {navItems.map(item => {
                    ...
                    return (
                        <button key={item.label} onClick={() => !item.isPlaceholder && navigateTo(item.screen)} className={`flex flex-col items-center justify-center gap-1 rounded-lg px-4 py-2 w-24 ${isActive ? 'text-primary-light' : 'text-muted-dark'}`}>
...
const PermissionsScreen: React.FC<ScreenProps> = ({ navigateTo }) => {
    ...
    const allGranted = granted.length === 3;
    ...
    <div className="relative flex min-h-screen w-full flex-col dark:bg-background-dark">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-sm">
            <div className="p-4 border-t border-muted-light/10 dark:border-muted-dark/20">
                <button onClick={() => navigateTo(Screen.SignIn)} disabled={!allGranted} className="flex w-full min-w-[84px] items-center justify-center overflow-hidden rounded-xl h-12 px-5 bg-primary text-background-dark text-base font-bold leading-normal disabled:bg-primary/50 disabled:text-background-dark/50 disabled:cursor-not-allowed">
                    <span className="truncate">Continue</span>
                </button>
